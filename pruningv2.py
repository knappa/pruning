...
res = minimize(
    ...
    bounds=[(0.0, None)] + [(1e-6, None)] * (num_tree_nodes - 1),
...
def prob_model_helper_vec(
    ...
    # return np.clip(
    #     ((np.exp(t[:, None] * evals)[:, None, :] * left[None, :, :]) @ right).astype(np.float64),
    #     0.0,
    #     1.0,
    # )
...
def compute_leaf_vec(patterns) -> Callable:
    ...
    result = np.array([arr[p, :] for p in patterns], dtype=np.float64)
...
def compute_score_function_helper(node, patterns, taxa_indices_) -> Callable:
    ...
    def local_score_function_branching(prob_matrices: np.ndarray) -> np.ndarray:
        ...
        p_l = prob_matrices[left_index, :, :]
        p_r = prob_matrices[right_index, :, :]

        v_n = (w_l[left_pattern_inverse] @ p_l) * (w_r[right_pattern_inverse] @ p_r)
        print(v_n)
        return v_n
...
def compute_score_function(*, root, patterns, pattern_counts) -> Callable:
    ...
    def score_function(prob_matrices):
        ...
        return -(pattern_counts @ np.log(v @ pis))[0]
