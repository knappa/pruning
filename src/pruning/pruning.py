...
model_list = ["DNA", "PHASED_DNA", "UNPHASED_DNA", "CELLPHY", "GTR10Z", "GTR10"]
...
match opt.model:
    case "DNA":
        ...
        pis_modification = lambda pis: pis
...
def neg_log_likelihood_prototype(
    pis, model_params, tree_distances, *, prob_model_maker, score_function, pis_modification
):
    prob_model = prob_model_maker(pis, model_params, vec=True)
    prob_matrices = prob_model(tree_distances)
    return score_function(pis_modification(pis), prob_matrices)
...
for _ in range(2):
    ...
    s_est = res.x / rate_constraint(pis_est, res.x)  # fine tune mu
