...
def fit_model(
    *,
    ...
    for _ in range(3):
        ...
        branch_lengths = np.clip(res.x, 0.0, np.inf)
        ...
        rate_params = np.clip(res.x, 0.0, np.inf)
    ...
    rate_params = res.x[:num_rate_params]
    branch_lengths = np.clip(res.x[num_rate_params:], 0.0, np.inf)
